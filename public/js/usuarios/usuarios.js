function usuarios(URI, ANIO_EJECUCION) {
	...
	function areasByUser(Anio_Ejecucion, Codigo_Sector, Codigo_Pliego, Codigo_Ejecutora, Codigo_Centro_Costos, Codigo_Sub_Centro_Costos, Codigo_Usuario) {
		...
		$.ajax({
			url: URI + 'usuarios/main/areasUsuario',
			method: 'post',
			type: 'json',
			data: {
			...
			if (id.length > 0) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						$.each(data.ofertamovil, function (i, e) {
						...
						$.each(data.emergenciasSanitarias, function (i, e) {
						...
						$.each(data.hospitalesSeguros, function (i, e) {
						...
						$.each(data.coronavirus, function (i, e) {
						...
						$.each(data.farmacia, function (i, e) {
